...
When the server is started with [`--query-timeout`](/docs/reference/cli/surrealdb-cli/environment-variables.md#command-environment-variables) (`SURREAL_QUERY_TIMEOUT`), the same duration also limits the wall-clock time of a whole request on these endpoints:

- [`POST /sql`](#sql) and the `/sql` WebSocket.
- [`POST /gql`](#gql).
- [`POST /graphql`](#graphql), for a single request and for a batch.
- `POST /rpc`, for every [RPC method](/docs/reference/rest-api/rpc-protocol.md), in the same way as on the WebSocket `/rpc` endpoint and over gRPC. This includes `signin`, `signup` and `authenticate`.
...
This HTTP RESTful endpoint checks whether the storage engine responds, by reading one key in a read-only transaction. It returns `200` when the read succeeds and `500` when it fails, or `403` when the `health` route is denied by [capabilities](/docs/learn/security/authorization/capabilities.md#http-route-names).
...
When [authentication rate limiting](/docs/reference/cli/surrealdb-cli/environment-variables.md#authentication-rate-limiting-environment-variables) is enabled, an attempt beyond the client address's budget is refused with `429 Too Many Requests` and a `Retry-After` header giving the number of seconds to wait, and the error body says that too many authentication attempts were made from this client address. `/signin` and [`/signup`](#signup) draw on the same budget. _(since v3.3.0)_
...
When [authentication rate limiting](/docs/reference/cli/surrealdb-cli/environment-variables.md#authentication-rate-limiting-environment-variables) is enabled, an attempt beyond the client address's budget is refused with `429 Too Many Requests` and a `Retry-After` header giving the number of seconds to wait. `/signup` shares its budget with [`/signin`](#signin). _(since v3.3.0)_
...
The GQL endpoint runs [ISO GQL (Cypher Query Language)](/docs/learn/querying/gql/overview.md) graph pattern queries against your existing tables and `RELATE` edges - `MATCH … RETURN` reads and data-modifying `INSERT`, `SET`, `REMOVE`, and `DELETE` ([GQL mutations](/docs/learn/querying/gql/mutations.md)).
...
> [!NOTE]
> This endpoint is **not** [ISO GQL (Cypher Query Language)](/docs/learn/querying/gql/overview.md). GQL `MATCH` queries belong on [`POST /gql`](#gql).
